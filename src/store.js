...
export default new Vuex.Store({
  ...
  actions: {
    ...
    CREATE_USER: ({ state, commit }, { email, name, lastname, docType, docNumber, contactNumber, password }) => new Promise((resolve) => {
      firebase.auth().createUserWithEmailAndPassword(email, password).then((account) => {
        const id = account.user.uid;
        const registeredAt = Math.floor(Date.now() / 1000);
        const newUser = { email, name, lastname, docType, docNumber, contactNumber, registeredAt };
